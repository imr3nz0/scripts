...
def create_or_upload_test_based_on_count(dojo_token, url, product_name, engagement_name, filename):
    ...
    # Verifica a existência de testes com o scan_type 'Gitleaks JSON Report' no engagement
    ...
    gitleaks_test_exists = any(test['test_type_name'] == 'Gitleaks Scan' for test in tests_data.get('results', []))

    if not gitleaks_test_exists:
        # Se não existir um teste com 'Gitleaks', cria um novo teste
        print("Criando um novo teste com scan_type 'Gitleaks Scan'.")
        new_test_data = {
...
if __name__ == "__main__":
    if len(sys.argv) == 9:
        ...
        print('Uso: python3 gitleaks.py --product NOME_DO_PRODUTO --engagement NOME_DO_ENGAGEMENT --report REPORT_SCAN --dojo_token TOKEN_DO_DOJO')
